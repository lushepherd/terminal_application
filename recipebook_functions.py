...
# Exit message function used to clear screen and print exit message either at the end of a function or when a user selects exit
...
def select_recipe(category, db):
    ...
    # Error handler for selected category being empty
...
def delete_recipe():
    ...
    if selected_recipe:
        ...
        # Delete the selected recipe from category's table
...
def search_recipes():
    ...
    """
    # Loop to validate user input. Will continue to prompt until a valid search term is provided or user cancels
    while True:
    ...
    # Creates a list of choices for to select from in alphabetical order
    recipe_choices = [Choice(value=index, name=recipe["recipe_name"])
                      for index, recipe in enumerate(sorted_recipes)]
    ...
    # Get selected recipe from sorted list
...
class PDF(FPDF):
    ...
    # Sets up header - image, font, cell content
    def header(self):
...
# Create a PDF
# Add a new page, set the title as the recipe name
...
def export_recipe():
    ...
    # Checks there is a recipe in category
